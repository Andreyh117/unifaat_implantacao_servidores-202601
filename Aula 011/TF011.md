...
3. **Instalação do Cliente PostgreSQL:** Print do comando `psql --version` mostrando a versão instalada.
...
2. **Conectar ao Banco de Dados:** Após a instância estar disponível (status: available), conecte-se usando o cliente PostgreSQL:
   ```
   psql -h <endpoint-rds> -U <usuario> -d <nome-banco-dados> -p 5432
   ```
